...
import { useRemoveMutation, useUploadMutation } from '@/data/upload';
...
export default function Uploader({
  ...
  const { mutate: remove, isLoading: loadingRemove } = useRemoveMutation();
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDrop: async (acceptedFiles) => {
      if (acceptedFiles.length) {
        upload(
          {
            files: acceptedFiles, // it will be an array of uploaded attachments
            id:
              !multiple && files && files.length > 0 ? files[0].id : undefined,
          },
          {
            onSuccess: (data: any) => {
